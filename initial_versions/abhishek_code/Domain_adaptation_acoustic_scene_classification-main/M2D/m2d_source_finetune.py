...
results_csv_path = f'./results_M2D_cdan+augment_a-{tgt_device}.csv'
weights_save_dir = './weights_M2D_cdan+augment_per_epoch'
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for batch_idx , (src_data , tgt_data)  in enumerate(create_combined_loader(src_loader, tgt_loader)):
        ...
        total_cls_loss += _loss_ASC.item()
        
        if (batch_idx + 1) % 50 == 0:
             print(f'Epoch [{epoch}/{NUM_EPOCHS}], Step [{batch_idx+1}], Loss: {_loss_ASC.item():.4f}')
