...
PATH = '/DATA/TAU-urban-acoustic-scenes-2020-mobile-development'
...
for dev in ['b' , 'c' , 's1' , 's2' , 's3']:
    ...
    results_csv_path = f'./results_M2D_source_cdan_a-{tgt_device}.csv'
    weights_save_dir = f'./weights_M2D_source_cdan_per_epoch_{tgt_device}'
    ...
    if RANDOM : 
        random_layer = RandomLayer([feature_dim, NUM_CLASS], feature_dim).to(device)
    else :
        random_layer = None
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        with open(results_csv_path, mode='a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch, src_device , f'{src_accuracy:.2f}', tgt_device , f'{tgt_accuracy:.2f}'])
