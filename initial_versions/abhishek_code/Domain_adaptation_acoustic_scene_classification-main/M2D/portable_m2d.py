...
class PortableM2D(torch.nn.Module):
    def __init__(self, weight_file, num_classes=None, freeze_embed=False, flat_features=None, use_spec_augment=True):
        super().__init__()
        self.cfg = Config()
        ...
        if use_spec_augment:
        # These are example parameters, you should tune them for your dataset
            self.spec_augment = torch.nn.Sequential(
                TimeMasking(time_mask_param=40),
                FrequencyMasking(freq_mask_param=8)
            )

        logging.info('** SpecAugment is ENABLED for training **')
        self.eval()
