...
# SED
#####
def evaluate_SED(model, evlset_gen, device, nb_mels , nb_frames_ASC ,  ):
    y_true = []
    y_pred = []
    y_keys = []
    model.eval()
...
# ASC
#####
def evaluate_ASC(model, evlset_gen, device, arguments):
    y_pred = []
    y_true = []
    model.to(device)
    model.eval()
    with torch.no_grad():
        for m_batch, m_label in evlset_gen:
            m_batch = m_batch.view(-1, 1, arguments['nb_mels'], arguments['nb_frames_ASC']+1).to(device)
    ...
    if arguments["verbose"] > 0: print('Eval ASC success')
