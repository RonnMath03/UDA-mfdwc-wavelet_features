...
results_csv_path = f'./results_PaSST_source_a-all_device.csv'
weights_save_dir = f'./weights_PaSST_source_per_epoch_all_device'
...
src_loader = DataLoader(src_dataset, batch_size=bs_passt, shuffle=True, num_workers=2)

tgt_dataset = SimpleAudioDataset(file_df=all_trn_tgt_df, root=PATH, target_sr=TARGET_SAMPLE_RATE)
tgt_loader = DataLoader(tgt_dataset, batch_size=bs_passt, shuffle=True, num_workers=2)

src_test_dataset = SimpleAudioDataset(file_df=test_src_df[src_device], root=PATH, target_sr=TARGET_SAMPLE_RATE)
src_test_loader = DataLoader(src_test_dataset, batch_size=bs_passt, shuffle=True, num_workers=2)

tgt_test_dataset = SimpleAudioDataset(file_df=all_tst_tgt_df, root=PATH, target_sr=TARGET_SAMPLE_RATE)
tgt_test_loader = DataLoader(tgt_test_dataset, batch_size=bs_passt, shuffle=True, num_workers=2)
...
len(source_loader) , len(tgt_loader)
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        if epoch % 1 == 0:
            ...
            with open(results_csv_path, mode='a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([epoch, src_device , f'{src_acc:.2f}', 'seen set' , f'{tgt_acc:.2f}'])
        ...
        if epoch % 1 == 0:
            torch.save({
                ...
                'src_acc': src_acc,
                'tgt_acc': tgt_acc
            }, os.path.join(weights_save_dir, f'model_epoch_{epoch}_acc_{tgt_acc:.4f}.pth'))
