...
class MFDWCFeatureExtractor(nn.Module):
    ...
    def __init__(self, n_mels=60, n_fft=2048, hop_length=256, 
        ...
        Args:
            ...
            return_temporal: Output mode selection
                           ...
                           - True: Time-frequency features for CNN
                                  features: (batch_size, 1, 90, n_frames) for n_mels=60
        ...
        Expected output dimensions (for n_mels=60):
            ...
            return_temporal=True (For CNN):
                - Output: (batch_size, 1, 90, n_frames) for n_mels=60
        """
    ...
    def forward(self, waveform):
        ...
        if self.return_temporal:
            ...
            # Output: (batch_size, 1, 90, n_frames) for n_mels=60
            temporal_features = stacked_features.transpose(1, 2)  # (batch, 90, n_frames)
            return temporal_features.unsqueeze(1)  # (batch, 1, 90, n_frames)
    ...
    def extract_mfdwc_frames(self, audio):
        ...
        for frame in windowed_frames:
            # 1. Compute FFT (STFT for single frame)
            fft_spectrum = np.fft.rfft(frame, n=self.n_fft)
            magnitude_spectrum = np.abs(fft_spectrum)**2
            
            # 2. Apply mel-filterbank to magnitude spectrum
            mel_energies = np.dot(self.mel_filters, magnitude_spectrum)
            
            # 3. Apply log scaling (with small epsilon to avoid log(0))
